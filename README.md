...
| **Front-end & Back-end Development** | Mobile: ``Dart`` ``Flutter`` ``Microsoft Azure`` ``PostgreSQL`` ``Git`` <br> Web: ``Python`` ``JavaScript`` ``React`` ``Django`` ``PostgreSQL`` ``HTML`` ``CSS`` ``Microsoft Azure`` ``Git`` |
| **AI/ML**                         | ``Python`` ``TensorFlow`` ``OpenCV`` ``Keras`` ``NLTK`` ``Streamlit`` ``Microsoft Azure`` |
##
## 🗂️ Portfolio
- **Projects:**
